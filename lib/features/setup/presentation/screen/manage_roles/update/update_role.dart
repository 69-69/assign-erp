...
/*
  ...
  void _removeDeletedPermissions() {
    final deletedPermissions = _role.permissions.difference(_touchedModules);

    for (final perm in deletedPermissions) {
      _assignedPermissions.removeWhere((p) => p.module == perm.module);
    }
  }

  // refactor _removeDeletedPermissions to remove deleted permissions and add new permission if toggled
  void _removeDeletedPermissionsAndAdd() {
    _removeDeletedPermissions();
    for (final perm in _touchedModules) {
      _assignedPermissions.removeWhere((p) => p.module == perm);
    }
  }

  // refactor _removeDeletedPermissionsAndAdd to not override existing permissions that are not unselected yet and add new permissions if toggled
  void _removeDeletedPermissionsAndAdd2() {
    _removeDeletedPermissions();
    for (final perm in _touchedModules) {
      _assignedPermissions.removeWhere((p) => p.module == perm);
    }
    for (final perm in _touchedModules) {
      _assignedPermissions.add(RolePermission(module: perm, name: ''));
    }
  }
