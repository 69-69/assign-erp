...
/// LIST SALES ORDERS
class ListFinance extends StatefulWidget {
  const ListFinance({super.key});

  @override
  State<ListFinance> createState() => _ListFinanceState();
}

class _ListFinanceState extends State<ListFinance> {
  // List to group orders for printout
  final List<Orders> _groupFinancesForPrintout = [];
      ...
      onAllChecked:
          (
            ...
            if (!isAllChecked.first) {
              setState(() => _groupFinancesForPrintout.clear());
            }
    ...
    return AdaptiveLayout(
      ...
      children: [
        ...
        _IssueMultiInvoicePrintout(
          orders: _groupFinancesForPrintout,
          onDone: (s) => setState(() => _groupFinancesForPrintout.clear()),
        ),
      ],
    );
  }
  ...
  void _onChecked(
    ...
    setState(() {
      ...
      if (isChecked != null && isChecked) {
        ...
        List<Orders> tempOrdersForInvoice = List.from(_groupFinancesForPrintout)
          ..add(order);
        ...
        if (r.isSame) {
          _groupFinancesForPrintout.add(order);
        } else {
          context.orderNumberMisMatchWarningDialog(r.misMatchID);
        }
      } else {
        _groupFinancesForPrintout.remove(order);
      }
    });
  }
